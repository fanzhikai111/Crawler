...
import main.java.edu.fiu.yxjiang.system.noiser.planner.Task;
...
public class Worker implements MessageListener {
	...
	public Worker() throws Exception {
		brokerAddress = "tcp://"
				+ SystemHelper.getSystemLocalIp().getHostAddress() + ":"
				+ SERVICE_PORT;
		startBroker();
		init();
	}
	...
	private void startBroker() {
		...
		try {
			...
			this.broker.start();
			System.out
					.printf("Broker service (%s) started...\n", brokerAddress);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	class NoiseExecutor extends Thread {
		...
		public void run() {
			...
			if (type.equals("cpu")) {
				System.out.printf(
						"At time (%d), execute [%s] noise for %d seconds.\n",
				...
				try {
					...
					if (false == shFile.exists()) {
						System.out
								.println("\tGenerate memory eater script on demand.");
						FileWriter fw = new FileWriter(shFile);
						StringBuffer scriptContent = new StringBuffer(
								"cd classes\n");
						scriptContent
								.append("memory=$(free -g | head -n2 | tail -n1 | tr -s ' ' | cut -d ' ' -f2)\n");
						scriptContent.append("unit=g\n");
						scriptContent
								.append("java -cp . -Xmx$memory$unit edu.fiu.yxjiang.system.noiser.worker.MemoryEater $1");
					...
					Runtime.getRuntime().exec(
							"sh memoryEater.sh " + durationInSec);
				} catch (IOException e) {
					e.printStackTrace();
				}
				System.out.printf(
